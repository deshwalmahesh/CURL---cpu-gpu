...
class ImageProcessing(object):
    ...
    def lab_to_rgb(img, is_training=True):
        ...
        img = torch.matmul(
            img + Variable(torch.cuda.FloatTensor([16.0, 0.0, 0.0]) if DEVICE == 'cuda' else torch.FloatTensor([16.0, 0.0, 0.0])), lab_to_fxfyfz)
        ...
        img = torch.mul(img, Variable(
            torch.cuda.FloatTensor([0.950456, 1.0, 1.088754])) if DEVICE == 'cuda' else torch.FloatTensor([0.950456, 1.0, 1.088754]))
